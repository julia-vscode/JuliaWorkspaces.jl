...
JW-owned wrapper `src/vendor_lowering.jl`, which mirrors
...
load on 1.11) and are used exclusively by `src/layer_lowering.jl`.
...
2. Re-sync `src/vendor_lowering.jl` against the new
