...
| `analysis_boundary` | `information` | A construct the linter cannot see through (an interpolated `@eval`, a runtime `eval`) silences the semantic rules in its module; this notice on the construct names them. Requires the lowering-lint flag |
