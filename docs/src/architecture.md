...
| `layer_project_files_v2.jl` | Full-fidelity `Project.toml`/`Manifest.toml` parse products (`JuliaProjectFile`, `JuliaManifestFile`) with position-free problem records for every malformed or inconsistent section (located via the TOML item walk at the diagnostics last mile). |
| `layer_workspaces_v2.jl` | `[workspace]` discovery: which folder is a member of which workspace, resolved upward to the outermost root the way Pkg does it. |
...
| `layer_extensions_v2.jl` | Package extensions: mapping `ext/` files to their `[extensions]` entry and finding an environment containing their `[weakdeps]` triggers (an existing covering manifest, else a resolved extension environment from a child process). |
