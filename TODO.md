# TODO

This is a list of open issues that we still need to address before we can switch things over
- Add cache cloud download back in
- Make sure cache location is properly configured
- Handle different Julia versions based on manifest
- Bring progress notifications back
- Have three options: no dynamic feature, only symbol indexing, permanent dynamic processes
- Make client process management more robust
- Handle watching of lazy inputs
- Consider whether we should switch to pull based diagnostics right away because that might be a better fit
- Figure formatting out
- Add lint config for static lint to lint config file
