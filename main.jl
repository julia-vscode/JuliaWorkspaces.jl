...
# jw = JuliaWorkspaces.workspace_from_folders([joinpath(homedir(), ".julia/dev/CSTParser")], dynamic=DynamicPersistent)

jw = JuliaWorkspaces.workspace_from_folders([joinpath(homedir(), ".julia/dev/DataFrames")], dynamic=DynamicPersistent)
...
diags = get_diagnostics_blocking(jw)
