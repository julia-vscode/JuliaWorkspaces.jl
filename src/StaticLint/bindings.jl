...
hasbinding(x::EXPR, meta_dict) = hasmeta(x, meta_dict) && hasbinding(getmeta(x, meta_dict))
bindingof(x, meta_dict) = nothing
bindingof(x::EXPR, meta_dict) = hasbinding(x, meta_dict) ? bindingof(getmeta(x, meta_dict)) : nothing


hasref(x::EXPR, meta_dict) = hasmeta(x, meta_dict) && hasref(getmeta(x, meta_dict))
refof(x::EXPR, meta_dict) = hasmeta(x, meta_dict) ? getmeta(x, meta_dict).ref : nothing


function gotoobjectofref(x::EXPR, meta_dict)
    r = refof(x, meta_dict)
...
function mark_bindings!(x::EXPR, state)
    ...
    elseif headof(x) === :for
        markiterbinding!(x.args[2], meta_dict)
    elseif headof(x) === :generator || headof(x) === :filter
        for i = 2:length(x.args)
            markiterbinding!(x.args[i], meta_dict)
        end
    elseif headof(x) === :do
        for i in 1:length(x.args[2].args)
            mark_binding!(x.args[2].args[i], meta_dict)
        end
    elseif headof(x) === :function || headof(x) === :macro
        name = CSTParser.get_name(x)
        getmeta(x, meta_dict).binding = Binding(name, x, CoreTypes.Function, [])
        if isidentifier(name) && headof(x) === :macro
            setref!(name, bindingof(x, meta_dict), meta_dict)
        end
        mark_sig_args!(CSTParser.get_sig(x), meta_dict)
    elseif CSTParser.defines_module(x)
        getmeta(x, meta_dict).binding = Binding(x.args[2], x, CoreTypes.Module, [])
        setref!(x.args[2], bindingof(x, meta_dict), meta_dict)
    elseif headof(x) === :try && isidentifier(x.args[2])
        mark_binding!(x.args[2], meta_dict)
...
function mark_sig_args!(x::EXPR, meta_dict)
    if CSTParser.iscall(x) || CSTParser.istuple(x)
        if x.args !== nothing && length(x.args) > 0
            if CSTParser.isbracketed(x.args[1]) && length(x.args[1].args) > 0 && CSTParser.isdeclaration(x.args[1].args[1])
                mark_binding!(x.args[1].args[1], meta_dict)
            end
    ...
    elseif CSTParser.isbinarycall(x)
        mark_binding!(x.args[1], meta_dict)
        mark_binding!(x.args[2], meta_dict)
...
function mark_typealias_bindings!(x::EXPR, meta_dict)
    ensuremeta(x, meta_dict)
    getmeta(x, meta_dict).binding = Binding(CSTParser.get_name(x.args[1]), x, CoreTypes.DataType, [])
    ...
    for i = 2:length(x.args[1].args)
        ...
        elseif CSTParser.issubtypedecl(arg) && isidentifier(arg.args[1])
            mark_binding!(arg.args[1], meta_dict)
        end
    end
    return x
end
                ...
                if lhs_ref isa SymbolServer.ModuleStore && haskey(lhs_ref.vals, Symbol(name))
                    ...
                    if haskey(tls.names, name) && eventually_overloads(tls.names[name], lhs_ref.vals[Symbol(name)], state)
                        ...
                        if !hasref(b.name, meta_dict)
                            setref!(b.name, tls.names[name], meta_dict) # Add ref to previous overload
...
# Nearest enclosing *local* scope (stopping before the global module/file scope)
# that already binds `name`, or nothing. Used to decide whether an assignment in
# a local scope reassigns a variable inherited from an enclosing local scope
# (let block / closure) rather than introducing a fresh local.
...
function name_extends_imported_method(b::Binding, meta_dict)
    if CoreTypes.isfunction(b.type) && CSTParser.hasparent(b.name) && CSTParser.is_getfield(parentof(b.name))
        if refof_maybe_getfield(parentof(b.name)[1], meta_dict) !== nothing
